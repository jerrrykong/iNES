...
**现象**：win32 发布后 mac 看不到，mac 发布后 win32 看得到（单向可见）。

**定位**：不是协议/编码问题 —— 两端 UI 与 `comm/lan.c` 的调用、报文布局（128 字节、`#pragma pack(1)`、
网络序、magic/ver 校验）完全对称，且 win32 能收到 mac 的 beacon 说明 win32 侧接收与解析都没问题。
落在 **win32 侧的出接口**：本机 `ipconfig` 有两个活动 IPv4 接口

| 接口 | 地址 | 掩码 | 默认网关 |
|---|---|---|---|
| `singbox_tun`（VPN 隧道） | 172.18.0.1 | 255.255.255.252 | **172.18.0.2** |
| `WLAN` | 192.168.10.29 | 255.255.255.0 | 192.168.10.1 |

`sendto()` 到有限广播 `255.255.255.255` 时，Windows 只走**一个**接口（默认路由/绑定顺序最靠前的那个），
于是 beacon 被发进了 VPN 隧道网段，`192.168.10.x` 的 mac 收不到；而 mac 只有一个 Wi-Fi 接口，
广播必然发到 `192.168.10.255`，win32 绑定 `0.0.0.0:8892` 全接口接收，所以反向可见。

**修复**（`comm/lan.c`，两端受益）：`lan_advertise()` 先枚举本机活动 IPv4 接口，
向每个接口的**子网定向广播地址**（`addr | ~mask`）各发一份；有限广播只在枚举失败或全部发送失败时兜底。

- win32：`WSAIoctl(SIO_GET_INTERFACE_LIST)`（只要已链接的 `ws2_32`，不引入 `iphlpapi`）。
- macOS / POSIX：`getifaddrs()`，过滤非 `AF_INET`、非 `IFF_UP`、非 `IFF_BROADCAST`、回环网段、`/32` 掩码。
- 调试日志（`LOG_DBG`）：`lan: advertise -> 192.168.10.255`；全失败只报一次 `LOG_ERR`。

排查同类问题的手段：mac 端 `sudo tcpdump -n -i en0 udp port 8892`（能看到 win32 的源 IP 即为已到达），
win32 端临时关掉 VPN / 禁用虚拟网卡是区分"出接口"与"防火墙"的最快办法。
