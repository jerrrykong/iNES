...
static void mapper213_reset(ines_mapper_t* p_mapper)
{
...
static void mapper213_writehigh(ines_mapper_t* p_mapper, ines_word_t addr, ines_byte_t  val)
{
	ines_host_t*  p_host = mapper2host(p_mapper);
	(void)p_host;
}
...
{
	p_mapper->reset = mapper213_reset;
	p_mapper->writehigh = mapper213_writehigh;
	return ines_false;
}
