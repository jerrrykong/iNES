...
static void mapper158_reset(ines_mapper_t* p_mapper)
{
...
static void mapper158_writehigh(ines_mapper_t* p_mapper, ines_word_t addr, ines_byte_t  val)
{
	ines_host_t*  p_host = mapper2host(p_mapper);
	(void)p_host;
}
...
{
	p_mapper->reset = mapper158_reset;
	p_mapper->writehigh = mapper158_writehigh;
	return ines_false;
}
